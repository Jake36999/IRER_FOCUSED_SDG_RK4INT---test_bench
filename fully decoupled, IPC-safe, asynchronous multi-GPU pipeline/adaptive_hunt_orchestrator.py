...
def aggregate_results(gen, jobs_to_run, config, hunter):
    ...
    for c_hash, p_path in successful_jobs:
        ...
        if os.path.exists(prov_file):
            try:
                # 1. Load Provenance to get the error
                with open(prov_file, 'r') as f:
                    prov = json.load(f)
                err = float(prov.get("spectral_fidelity", {}).get("primary_harmonic_error", 999.0))
                
                # 2. Load the original parameters to check the runtime context
                with open(p_path, 'r') as pf:
                    job_params = json.load(pf)
                run_origin = job_params.get("origin", "NATURAL")
                
                # 3. CONTEXT-AWARE FSS EXECUTION
                if err < 0.2:
                    if run_origin == "BACKLOG_RE_SIM":
                        logging.info(f"🌟 GOLDEN RUN ({c_hash[:10]}). Origin: BACKLOG. Bypassing FSS to preserve $96^3$ Phase Diagram data.")
                        # FSS Skipped. Data remains pristine.
                    else:
                        logging.info(f"🌟 GOLDEN RUN DETECTED ({c_hash[:10]}). Origin: {run_origin}. Executing Sequential FSS Check...")
...
def main():
    ...
    for gen in range(start_gen, config["generations"]):
        ...
        if os.path.exists(backlog_file):
            try:
                with FileLock(f"{backlog_file}.lock", timeout=15):
                    ...
                    if backlog:
                        ...
                        tmp_file = backlog_file + ".tmp"
                        with open(tmp_file, "w", encoding="utf-8") as f:
                            json.dump(remaining_backlog, f, indent=4)
                        os.replace(tmp_file, backlog_file)
